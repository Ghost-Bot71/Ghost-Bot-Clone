...
// Ghost Bot: Gmail credentials are optional — skip if not provided
if (clientId && !clientSecret) {
        log.warn("CREDENTIALS", `Ghost Bot: clientSecret missing in ${path.normalize(global.client.dirConfig)} — Gmail disabled`);
}
if (clientId && !refreshToken) {
        log.warn("CREDENTIALS", `Ghost Bot: refreshToken missing in ${path.normalize(global.client.dirConfig)} — Gmail disabled`);
}
...
if (clientId && clientSecret && refreshToken) {
        ...
        log.info("CREDENTIALS", "✅ Ghost Bot: Gmail/Drive credentials loaded");
} else {
        log.warn("CREDENTIALS", "👻 Ghost Bot: Gmail not configured — Drive features disabled, bot continues normally");
}
const word = [
...
const drive = {
        ...
        async uploadFile(fileName, mimeType, file) {
                if (this._noGmail) throw new Error("👻 Ghost Bot: Google Drive not configured (Gmail credentials missing).");
        ...
        async deleteFile(id) {
                if (this._noGmail) throw new Error("👻 Ghost Bot: Google Drive not configured.");
