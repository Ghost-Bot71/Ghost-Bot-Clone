...
// FBSTATE env var support (Ghost Bot)
if (process.env.FBSTATE && !loginWithEmail) {
  try {
    const _fbstateData = process.env.FBSTATE.trim();
    if (_fbstateData && _fbstateData !== "" && _fbstateData !== "[]") {
      writeFileSync(global.client.dirAccount, _fbstateData);
      log.info("FBSTATE", "Loaded fbstate from FBSTATE environment variable");
    }
  } catch(e) { log.warn("FBSTATE", "Could not load FBSTATE env var: " + e.message); }
}
	let appState = [];
	if (loginWithEmail)
