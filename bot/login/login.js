// set bash title
process.stdout.write("\x1b]2;Ghost Bot - Owner: Rakib Islam\x1b\x5c");
...
// 👻 Ghost Net — Dynamic FCA Loader (Rakib Islam)
let login;
try {
  const _fs = defaultRequire("fs");
  const _path = defaultRequire("path");
  const _activeFile = _path.join(process.cwd(), "fca-modules", "active.json");
  let _activeName = "fca1";
  if (_fs.existsSync(_activeFile)) {
    try { _activeName = (JSON.parse(_fs.readFileSync(_activeFile, "utf8")).active || "fca1").trim(); } catch(e) {}
  }
  const _fcaDir = _path.join(process.cwd(), "fca-modules", _activeName);
  if (!_fs.existsSync(_fcaDir)) throw new Error("FCA dir not found: " + _activeName);
  login = defaultRequire(_fcaDir);
  console.log(`\x1b[36m[FCA-LOADER] Active FCA: ${_activeName}\x1b[0m`);
} catch(e) {
  console.warn(`\x1b[33m[FCA-LOADER] Fallback to built-in fb-chat-api (${e.message})\x1b[0m`);
  login = defaultRequire(`${process.cwd()}/fb-chat-api`);
}
...
const titles = [
        ...
        [
                "G H O S T  B O T @" + currentVersion
        ],
        [
                "GHOST BOT"
        ]
...
let subTitle = `Ghost Bot@${currentVersion} — Owner: Rakib Islam | Ghost Net Edition`;
const subTitleArray = [];
...
const author = ("Powered by Ghost Bot — Owner: Rakib Islam");
const srcUrl = ("Ghost Bot — Ghost Net Edition");
const fakeRelease = ("Ghost Bot is fully operational");
...
process.exit(); // Ghost Bot: kept for critical auth error
                                        }
...
// FBSTATE env var support (Ghost Bot) — tries FBSTATE, FBSTATE2, FBSTATE3
const _fbstateVars = ["FBSTATE", "FBSTATE2", "FBSTATE3"];
let _fbstateLoaded = false;
if (!loginWithEmail) {
  for (const _varName of _fbstateVars) {
    if (process.env[_varName]) {
      try {
        const _fbstateData = process.env[_varName].trim();
        if (_fbstateData && _fbstateData !== "" && _fbstateData !== "[]") {
          writeFileSync(global.client.dirAccount, _fbstateData);
          log.info("FBSTATE", `Loaded fbstate from env var: ${_varName}`);
          _fbstateLoaded = true;
          break;
        }
      } catch(e) { log.warn("FBSTATE", `Could not load ${_varName}: ` + e.message); }
    }
  }
}
        let appState = [];
        ...
        // 3 account.txt slot fallback: account.txt → account2.txt → account3.txt
        const _accountSlots = [
          require("path").join(process.cwd(), "account.txt"),
          require("path").join(process.cwd(), "account2.txt"),
          require("path").join(process.cwd(), "account3.txt"),
        ];
        let _resolvedAccount = null;
        for (const _slot of _accountSlots) {
          if (existsSync(_slot)) {
            const _slotText = readFileSync(_slot, "utf8").trim();
            if (_slotText && _slotText !== "[]" && _slotText !== "") {
              _resolvedAccount = _slot;
              if (_slot !== dirAccount) {
                log.info("LOGIN FACEBOOK", `Using account slot: ${require("path").basename(_slot)}`);
              }
              break;
            }
          }
        ...
        const accountText = readFileSync(_resolvedAccount, "utf8");
...
log.warn("LOGIN", "Login failed - waiting for fbstate in account.txt"); return null;
                }
        }
        return appState;
}
...
try { const _v = await axios.get("https://raw.githubusercontent.com/ntkhang03/Goat-Bot-V2-Storage/main/tooOldVersions.txt",{timeout:5000}); tooOldVersion=_v.data||"0.0.0"; } catch(_e){log.warn("VERSION","Ghost Bot: version check skipped");}
// Ghost Bot: version gate bypassed for stability
/*
process.exit(); */
// Ghost Bot: version check block end
        /* { CHECK ORIGIN CODE } */
...
log.warn("LOGIN", "Ghost Bot: dashboard disabled and no email/pass - check account.txt"); return;
                                }
                        }
...
if (hasBanned == true) { hasBanned=false; log.warn("GBAN","Ghost Bot: ignoring ban - continuing..."); }
// process.exit() bypassed
                        }
                        catch (e) {
                                console.log(e);
log.warn("GBAN","Ghost Bot: GBAN network error - continuing...");
// process.exit() bypassed;
                        }
                        // ———————————————— NOTIFICATIONS ———————————————— //
                        let notification;
...
log.warn("NOTIFICATION","Ghost Bot: notification skipped");
notification = "👻 Ghost Bot Online | Owner: Rakib Islam";
                        }
...
hasBanned = false; log.warn("GBAN","Ghost Bot: ignoring ban - continuing...");
                        }
                        // ——————————————————— LOAD DATA ——————————————————— //
