...
(async () => {
        // Ghost Bot: Google OAuth is optional - skip if credentials not configured
        ...
        if (email && clientId && clientSecret && refreshToken) {
                try {
                        ...
                        async function sendMail({ to, subject, text, html, attachments }) {
                                const t = nodemailer.createTransport({ host: 'smtp.gmail.com', service: 'Gmail', auth: { type: 'OAuth2', user: email, clientId, clientSecret, refreshToken, accessToken } });
                                return await t.sendMail({ from: email, to, subject, text, html, attachments });
                        }
                        global.utils.sendMail = sendMail;
                        global.utils.transporter = transporter;
                } catch(err) {
                        utils.log.warn("GMAIL", "Gmail OAuth skipped: " + err.message);
                        global.utils.sendMail = async () => { throw new Error("Gmail not configured"); };
                }
        } else {
                utils.log.warn("GMAIL", "Ghost Bot: Gmail not configured - email features disabled");
                        global.utils.sendMail = async () => { throw new Error("Gmail not configured"); };
        }
        ...
        // Ghost Bot: Google Drive is optional — skip if Gmail not configured
        if (!utils.drive._noGmail) {
                try {
                        const parentIdGoogleDrive = await utils.drive.checkAndCreateParentFolder("GoatBot");
                        utils.drive.parentID = parentIdGoogleDrive;
                } catch(_de) { utils.log.warn("DRIVE", "Ghost Bot: Drive setup skipped — " + _de.message); }
        } else {
                utils.log.warn("DRIVE", "👻 Ghost Bot: Google Drive disabled (no Gmail credentials)");
        }
