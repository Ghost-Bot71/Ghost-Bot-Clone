                        ...
                        .then(function (resData) {
                                ...
                                if (resData.error) {
                                        if (resData.error === 1545012) {
                                                ...
                                                // Facebook rate limit — "You Can't Use This Feature Right Now"
                                                // Suppress the full verbose dump, just log a brief warning
                                                log.warn("sendMessage", "⚠️ Facebook rate limit (1390008) on thread " + threadID + " — bot is sending too fast. Cooling down 15s...");
                                                // Delay and retry once after cooldown
                                                setTimeout(() => {
                                                        send(form, threadID, messageAndOTID, callback, isGroup);
                                                }, 15000);
                                                return;
                                        }
                                        else if (resData.errorSummary === "You Can't Use This Feature Right Now" || (resData.errorDescription && resData.errorDescription.__html && resData.errorDescription.__html.includes("limit"))) {
                                                log.warn("sendMessage", "⚠️ Facebook feature limit — cooling down 30s before retry...");
                                                setTimeout(() => {
                                                        send(form, threadID, messageAndOTID, callback, isGroup);
                                                }, 30000);
                                                return;
                                                ...
                                                log.warn("sendMessage", `Error ${resData.error || "unknown"} on thread ${threadID}`);
                                        }
